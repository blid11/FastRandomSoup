...
void DetectFalsePositive::shiftCellAddresses() {
  ...
  for (vector<short>::iterator iter = objectCellAddresses.begin();
    ...
    // cout<<"indices pre shift: ("<< indicesHolder[0] <<", "<<indicesHolder[1]
    // <<", "<< indicesHolder[2] << ")";
    ...
    // cout<< "shifted cell index: ("<< addAddressToVector[0] <<",
    // "<<addAddressToVector[1] <<", "<< addAddressToVector[2] << ")";
...
vector<short> DetectFalsePositive::getIndexVector(short convertThis) {
  ...
  // this function will take an int, and put its binary representation in a
  // cstring the char array is set to {0,0,0,0,0} to start the first value is
  // placed in the last spot of the array
  ...
  // will fill the 5th position with a 0 even if the quotient and remainder have
  // already been 0 this will be allow for easy conversion back to decimal later.
  while (counter != -1) {
  ...
  // for the first 5 values after 0th position, take the no in binary and make
  // it a short place the result in the first position of the vector(3)

  short x = 0;
  short y = 0;
  short z = 0;
  x = +(copyToArray[5]) * 1 + (copyToArray[4]) * 2 + (copyToArray[3]) * 4 +
      (copyToArray[2]) * 8 + (copyToArray[1]) * 16;
  y = +(copyToArray[10]) * 1 + (copyToArray[9]) * 2 + (copyToArray[8]) * 4 +
      (copyToArray[7]) * 8 + (copyToArray[6]) * 16;
  z = +(copyToArray[15]) * 1 + (copyToArray[14]) * 2 + (copyToArray[13]) * 4 +
      (copyToArray[12]) * 8 + (copyToArray[11]) * 16;
...
void DetectFalsePositive::runExperiment() {
  ...
  for (vector<long>::iterator iterLive = liveCellPrev.begin(); iterLive !=
  liveCellPrev.end(); iterLive++) { printThis = *iterLive; cout << printThis <<
  endl;
  }
  */
  ...
  while ((!hitBorder) && numLiveCells && (!oscillator) && (!stillLife)) {
    ...
    for (vector<long>::iterator iterLive = liveCellCurrent.begin(); iterLive !=
    liveCellCurrent.end(); iterLive++) { printThis = *iterLive; cout <<
    printThis << endl;
    }
    cout << endl;
    */
...
void DetectFalsePositive::makeNewSim() {
  ...
  for (iter = shiftedAddresses.begin(); iter != shiftedAddresses.end();
       iter++) {
    encodeThis = *iter;
    ...
    if (iter2 != relevantCells.end()) {
      ...
      // the state should now be set to on, its neighbour count does not
      // increase as whatever live cells have added it as a neighbour have
  ...
  while (updateStatCount != relevantCells.end()) {
    ...
    if (state && ((count == 5) || (count == 6) || (count == 7))) {
      ...
      for (int nbour = 0; nbour < 26; nbour++) {
        ...
        // neighbour index iterator points to the map of cells to increment and
        // decrement
      ...
      updateStatCount->second = statCount;
      // cout<<index<<"born, state: "<< state << ", nCount: "<<count<< ",
      // statcount" <<statCount<<endl; this works fine
...
bool DetectFalsePositive::getFalsePosInfo() { return falsePositive; }
